...
def main() -> None:
    ...
    for strat in STRATEGIES:
        ...
        for sym in COINS:
            for days in WINDOWS:
                try:
                    r = bl.run_backtest(symbol=sym, days=days, start_equity=EQUITY)
                    ...
                    print(f"{strat:12s} {sym:9s} {days:>4}d  trades={r.n:>3} "
                    ...
                    rows.append({"strategy": strat, "symbol": sym, "days": days,
                                 "error": str(e)[:60]})
                    print(f"{strat:12s} {sym:9s} {days:>4}d  ERR {e}", flush=True)
